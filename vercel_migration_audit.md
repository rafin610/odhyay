...
The corrected static deployment completed successfully but `/api/trpc/*` still returned Vercel’s `404 NOT_FOUND`, confirming that the static Vite framework preset was serving the public assets without dispatching the root Express server entrypoint. The project framework preset was therefore changed and saved as **Express**, retaining the explicit `pnpm build:vercel` build command and `public` output directory. This routes the Vercel deployment through the supported Express server model while preserving the static artifact directory. The next deployment will determine whether `/api` traffic now reaches the ODHYAY application server.
