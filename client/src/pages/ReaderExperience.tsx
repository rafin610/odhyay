...
export default function ReaderExperience() {
  ...
  const setSafePage = useCallback((nextPage: number) => setPage(Math.max(1, Math.min(pages, nextPage))), [pages]);
  const advanceFromScroll = useCallback((change: -1 | 0 | 1) => {
    if (change) setSafePage(page + change);
  }, [page, setSafePage]);
  const handleWheel = useCallback((event: React.WheelEvent<HTMLDivElement>) => {
    if (!isFullscreen || Math.abs(event.deltaY) < 4) return;
    event.preventDefault();
    const next = consumeContinuousScroll(scrollAccumulator.current, event.deltaY);
    scrollAccumulator.current = next.accumulated;
    advanceFromScroll(next.change);
  }, [advanceFromScroll, isFullscreen]);
  const handleTouchStart = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    if (isFullscreen) touchStartY.current = event.touches[0]?.clientY ?? null;
  }, [isFullscreen]);
  const handleTouchEnd = useCallback((event: React.TouchEvent<HTMLDivElement>) => {
    if (!isFullscreen || touchStartY.current === null) return;
    const endY = event.changedTouches[0]?.clientY;
    if (typeof endY === "number") advanceFromScroll(touchPageChange(touchStartY.current, endY));
